  ...
  const fetchQuota = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setQuota(data);
        if (data.billingModel !== 'wallet' && data.remaining === 0 && onQuotaExhausted) {
          onQuotaExhausted();
        }
      } else {
        setError(true);
  ...
  if (error) {
    return (
      <div className="w-full px-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs flex items-center justify-between gap-2">
        <span className="text-slate-400">Không thể tải số dư</span>
  ...
  if (quota.billingModel === 'wallet') {
    ...
    return (
      <div className={`w-full px-3 py-2.5 rounded-xl border text-xs flex flex-col gap-1.5 ${
        isLow ? 'bg-amber-50 border-amber-200' : 'bg-emerald-50 border-emerald-200'
      }`}>
        <div className="flex items-center justify-between">
          <span className={`font-semibold ${isLow ? 'text-amber-700' : 'text-emerald-700'}`}>
            Số dư ví
          </span>
          <span className={`font-bold ${isLow ? 'text-amber-700' : 'text-emerald-800'}`}>
